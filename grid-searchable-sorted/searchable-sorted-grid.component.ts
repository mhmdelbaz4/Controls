...
import { SearchableSortedGridModel } from './searchable-sorted-grid.model';
...
@Component({
  selector: 'ctrl-searchable-sorted-grid',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './searchable-sorted-grid.component.html',
  styleUrl: './searchable-sorted-grid.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SearchableSortedGridComponent {
  readonly currentPage = signal(1);
  readonly sortColumn = signal<string | null>(null);
  readonly sortDir = signal<'asc' | 'desc'>('asc');
  readonly searchQuery = signal('');

  private readonly _grid = signal<SearchableSortedGridModel | null>(null);

  @Input({ required: true })
  set grid(value: SearchableSortedGridModel) {
  ...
  /** Rows that match the search string in any column (case-insensitive substring). */
  readonly filteredElements = computed(() => {
    ...
    return list.filter((row) =>
      keys.some((key) => this.cellValue(row, key).toLowerCase().includes(q)),
    );
  });
